...
**Your Answers here:**
<br>
1. The returns of Apple are downloaded through the dataSet1.R file. The companies whose returns are downloaded are defined in the mytickers variable.
<br>
...
<br>
<br>
3. The cumulative returns of Yahoo during this period is `r round(100*sum(StockReturns[,3]),1)`%.
<br>
The average daily returns of Yahoo during this period is `r round(100*mean(StockReturns[,3]),3)`%;
<br>
The standard deviation of the daily returns of Yahoo during this period is `r round(100*sd(StockReturns[,3]),3)`%;
<br>
<br>
...
**Your Answers here:**
<br>
1.We can use nrow(transposedData) to get the number of rows and ncol(transposedData) to get the number of columns in the matrix transposedData
<br>
<br>
2.
```{r echo=FALSE, comment=NA, warning=FALSE, message=FALSE,results='asis',fig.align='center', fig=TRUE}
portfolio1 = apply(StockReturns,1,mean)
names(portfolio1) <- rownames(StockReturns)
pnl_plot(portfolio1)
```
...
**Your Answers here:**
<br>
1. Change the value stored in startDate to 2001-01-01
<br>
<br>
2. The portfolio equally comprised of Apple, Yahoo, Google and the S&P500 is displayed below
<br>
```{r echo=FALSE, comment=NA, warning=FALSE, message=FALSE,results='asis',fig.align='center', fig=TRUE}
portfolio1 = apply(StockReturns,1,mean)
names(portfolio1) <- rownames(StockReturns)
